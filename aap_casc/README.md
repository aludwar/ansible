...
* `env.yml` - File for environment variables to export (Ansible Galaxy / Automation Hub token) _(Encrypt this with Ansible Vault if you plan to commit to a public repository/source control)_
* `configs/auth.yml` - File for passwords, credentials, etc.  _(Encyrpt this with Ansible Vault if you plan to commit to a public repository/source control)_
